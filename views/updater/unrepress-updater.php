...
if (isset($_GET['force-check']) && $_GET['force-check'] == 1) {
    ...
    if (!wp_verify_nonce($nonce, 'upgrade-core')) {
        wp_die(__('Security check failed.', 'unrepress'));
    }
...
?>
<div class="wrap">
    <h1><?php esc_html_e('UnrePress Updates', 'unrepress'); ?></h1>
    <p><?php printf(esc_html__('Last checked on %s.', 'unrepress'), $wpLastChecked); ?></p>

    <section class="updates-core">
        <h2><?php esc_html_e('WordPress Core', 'unrepress'); ?></h2>
        ...
        <?php else: ?>
            <p><?php esc_html_e('Your WordPress installation is up to date.', 'unrepress'); ?></p>
        <?php endif; ?>
    </section>
